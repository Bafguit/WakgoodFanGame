...
public class ApplyStatusAction extends AbstractAction {
  ...
  public ApplyStatusAction(
      AbstractStatus status, AbstractEntity actor, AbstractEntity target, boolean fast) {
  ...
  public ApplyStatusAction(
      AbstractStatus status, AbstractEntity actor, Array<AbstractEntity> target, boolean fast) {
    super(actor, fast ? 0.25f : DUR_DEFAULT);
    this.status = status;
    this.target = target;
  }
