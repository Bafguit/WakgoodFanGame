...
public class ModeButton extends AbstractUI {
    ...
    private final Sprite locked = FileHandler.getUi().get("MODE_LOCK");
    private float a = aa;
    ...
    public ModeButton(AbstractLabyrinth.Mode diff) {
        ...
        this.mode = diff;
        isPixmap = true;
        clickable = mode == AbstractLabyrinth.Mode.NORMAL || UnlockHandler.unlocks.get(UnlockHandler.Unlocks.DIFF).get("HARD");
    }
    ...
    protected void onClick() {
        if(mode == AbstractLabyrinth.Mode.NORMAL) {
            SoundHandler.playSfx("CHANGE");
            Labyrintale.fadeOutAndChangeScreen(Labyrintale.diffScreen, Labyrintale.FadeType.VERTICAL, 0.5f);
        } else {
