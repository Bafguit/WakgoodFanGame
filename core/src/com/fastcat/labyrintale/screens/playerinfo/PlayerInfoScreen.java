        ...
        for(int g = 0; g < 2; g++) {
            for (int f = 0; f < 2; f++) {
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    b.setPosition(w * (0.175f + 0.46f * g + 0.06f * i) - b.sWidth / 2, h * (0.7f - 0.275f * f));
                    deck[c][i] = b;
                }
                for (int i = 0; i < 2; i++) {
                    PlayerInfoItemIcon b = new PlayerInfoItemIcon(player.item[i]);
                    b.setPosition(w * (0.365f + 0.46f * g + 0.06f * i) - b.sWidth / 2, h * (0.7f - 0.275f * f));
                    item[c][i] = b;
                }
                ...
                for (int j = 3; j >= 0; j--) {
                    for (int i = 1; i >= 0; i--) {
                        ...
                        s.entity = player;
                        s.setPosition(w * (0.398f + 0.46f * g - 0.055f * i), h * (0.8f - 0.275f * f + 0.027f * j));
                        stats[c][cnt++] = s;
                    }
                }
    ...
    public void render(SpriteBatch sb) {
        ...
        for(int f = 0; f < 2; f++) {
            for (int g = 0; g < 2; g++) {
                ...
                shr.rect(w * (0.15f + 0.46f * f), h * (0.83f - 0.275f * g), Math.max(w * 0.12f * ((float) player.health / (float) player.maxHealth), 0), h * 0.03f);
            }
        }
        shr.end();
        sb.begin();
        ...
        for(int f = 0; f < 2; f++) {
            for (int g = 0; g < 2; g++) {
                AbstractPlayer player = AbstractLabyrinth.players[cnt++];
                FontHandler.renderLineLeft(sb, fontName, player.name, w * (0.15f + 0.46f * f), h * (0.89f - 0.275f * g), w * 0.12f, 50);
                FontHandler.renderCenter(sb, fontHp, player.health + "/" + player.maxHealth, w * (0.15f + 0.46f * f), h * (0.847f - 0.275f * g), w * 0.12f, h * 0.03f);
            }
        }
