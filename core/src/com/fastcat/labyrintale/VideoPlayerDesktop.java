 ...
 */
public abstract class VideoPlayerDesktop implements VideoPlayer {

  private static final long SYNC = 350;
  ...
  abstract Music createMusic(
      VideoDecoder decoder, ByteBuffer audioBuffer, int audioChannels, int sampleRate);

  public boolean play(FileHandle file, float volume) throws FileNotFoundException {
    vol = volume;
    return play(file);
  }
    ...
    try {
      ...
      if (buffers != null) {
        ...
        if (audioBuffer != null) {
          ...
          audio =
              createMusic(
                  decoder, audioBuffer, buffers.getAudioChannels(), buffers.getAudioSampleRate());
          audio.setVolume(0);
        }
        currentVideoWidth = buffers.getVideoWidth();
        currentVideoHeight = buffers.getVideoHeight();
        if (texture != null
            && (texture.getWidth() != currentVideoWidth
                || texture.getHeight() != currentVideoHeight)) {
          texture.dispose();
          texture = null;
        }
      } else {
  ...
  public boolean update() {
    if (decoder != null && !paused && playing) {
      ...
      if (!showAlreadyDecodedFrame) {
        ...
        if (videoData != null) {
          if (texture == null)
            texture = new Texture(currentVideoWidth, currentVideoHeight, Format.RGB888);
          texture.bind();
          Gdx.gl.glTexImage2D(
              GL20.GL_TEXTURE_2D,
              0,
      ...
      if (audioFadeOut && audio != null && audioStart) {
        multi -= Labyrintale.tick;
        if (multi < 0) multi = 0;
        setVolume(vol * multi);
      }
      if (currentVideoTime >= SYNC && !audioStart && videoStart) {
        audioStart = true;
        if (audio != null) {
          audio.play();
          setVolume(vol);
  ...
  /**
   * Will return whether the buffer is filled. At the time of writing, the buffer used can store 10
   * frames of video. You can find the value in jni/VideoDecoder.h
   *
